...
/**
 * 将 Qdrant 版本字符串解析为各组成部分
 * 支持 "1.16.3"、"1.16"、"v1.16.3" 等格式
 */
...
/**
 * 比较两个 Qdrant 版本
 * 返回值: a < b 返回 -1，a == b 返回 0，a > b 返回 1，任一版本无法解析时返回 null
 */
...
export function isVersionCompatible(
  ...
  if (backup.major > restore.major) {
    return {
      compatible: false,
      warning: `无法将 Qdrant ${backupVersion} 快照恢复到 ${restoreVersion} 服务器。备份来自更新的主版本。`,
    }
  }
  ...
  return {
    compatible: true,
    warning: `正在将 Qdrant ${backupVersion} 快照恢复到 ${restoreVersion} 服务器。Qdrant 将在下次保存时升级快照格式。`,
  }
}
