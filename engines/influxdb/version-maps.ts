 ...
 * InfluxDB 版本映射
 *
 * 对 `hostdb` npm 包的轻量封装。架构设计原理参见
 * engines/sqlite/version-maps.ts —— hostdb 是唯一的版本信息来源。
 */
...
export function normalizeVersion(version: string): string {
  ...
  if (!isValidFormat) {
    logDebug(
      `InfluxDB 版本 '${version}' 格式无效，可能在 hostdb 中不可用`,
    )
  } else {
    logDebug(
      `InfluxDB 版本 '${version}' 不在 hostdb 中，可能无法下载`,
    )
  }
  return version
}
