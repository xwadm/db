#!/usr/bin/env tsx
/**
 * 生成包含示例数据的 MariaDB 数据库。
 *
 * 用法:
 *   pnpm generate:db mariadb [容器名称] [--port <端口>]
 */
...
async function main(): Promise<void> {
  ...
  console.log('等待 MariaDB 就绪...')
  const isReady = await waitForReady(containerName, ['--', '-e', 'SELECT 1'])

  if (!isReady) {
    console.error('错误: MariaDB 在规定时间内未就绪')
    process.exit(1)
  }

  console.log('MariaDB 已就绪。\n')
