#!/usr/bin/env tsx
/**
 * 生成包含示例数据的 FerretDB 数据库。
 *
 * 用法:
 *   pnpm generate:db ferretdb [容器名称] [--port <端口>] [--version <版本>]
 */
...
async function main(): Promise<void> {
  ...
  if (!isReady) {
    console.error('错误: FerretDB 在规定时间内未就绪')
    process.exit(1)
  }

  console.log('FerretDB 已就绪。\n')

  console.log('正在用示例数据填充数据库...')

  // FerretDB 使用 mongosh 配合 --file 运行 JS 种子脚本
  ...
  if (verifyResult.status === 0) {
    ...
    if (match) {
      console.log(`已验证: test_user 集合中有 ${match[1]} 个用户`)
    }
  }
