#!/usr/bin/env tsx
/**
 * 生成包含示例数据的 CockroachDB 数据库。
 *
 * 用法:
 *   pnpm generate:db cockroachdb [容器名称] [--port <端口>]
 */
...
async function main(): Promise<void> {
  ...
  console.log('CockroachDB 数据库生成器')
  console.log('=========================\n')
  ...
  console.log('等待 CockroachDB 就绪...')
  const isReady = await waitForReady(containerName, ['--', '-e', 'SELECT 1'])

  if (!isReady) {
    console.error('错误: CockroachDB 在规定时间内未就绪')
    process.exit(1)
  }

  console.log('CockroachDB 已就绪。\n')
