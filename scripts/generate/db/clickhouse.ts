#!/usr/bin/env tsx
/**
 * 生成包含示例数据的 ClickHouse 数据库。
 *
 * 用法:
 *   pnpm generate:db clickhouse [容器名称] [--port <端口>]
 */
...
async function main(): Promise<void> {
  ...
  if (verifyResult.status === 0) {
    ...
    if (match) {
      console.log(`已验证: test_user 表中有 ${match} 个用户`)
    }
  }
