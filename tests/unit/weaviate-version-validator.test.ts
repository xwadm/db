...
describe('Weaviate 版本验证器', () => {
  describe('parseVersion', () => {
    it('应解析完整版本字符串', () => {
      ...
      assertEqual(parsed!.major, 1, '主版本应为 1')
      assertEqual(parsed!.minor, 35, '次版本应为 35')
      assertEqual(parsed!.patch, 7, '补丁版本应为 7')
      assertEqual(parsed!.raw, '1.35.7', '原始字符串应为 1.35.7')
    })
  ...
  describe('isVersionSupported', () => {
    it('应支持 1.x 版本', () => {
      assert(isVersionSupported('1.35.7'), '版本 1.35.7 应受支持')
    ...
    it('不应支持 0.x 版本', () => {
      assert(!isVersionSupported('0.9.0'), '版本 0.9.0 不应受支持')
    })
  })

  describe('getMajorVersion', () => {
    it('应提取主版本', () => {
      assertEqual(getMajorVersion('1.35.7'), '1', '应提取 1')
  ...
  describe('getMajorMinorVersion', () => {
    it('应提取主版本.次版本', () => {
      assertEqual(getMajorMinorVersion('1.35.7'), '1.35', '应提取 1.35')
  ...
  describe('compareVersions', () => {
    it('应比较相等的版本', () => {
      assertEqual(compareVersions('1.35.7', '1.35.7'), 0, '相等版本')
    })
    ...
    it('对无效版本应返回 null', () => {
      assertEqual(compareVersions('invalid', '1.35.7'), null, '第一个无效')
      assertEqual(compareVersions('1.35.7', 'invalid'), null, '第二个无效')
    })
  })
  ...
  describe('isVersionCompatible', () => {
    ...
    it('从较旧主版本升级时应警告', () => {
      ...
      assert(result.warning !== undefined, '版本升级应有警告')
    })
  ...
  describe('isValidVersionFormat', () => {
    it('应验证正确格式', () => {
      assert(isValidVersionFormat('1.35.7'), '完整版本应为有效')
      assert(isValidVersionFormat('1.35'), '主版本.次版本应为有效')
      assert(isValidVersionFormat('1'), '仅主版本应为有效')
    })

    it('应拒绝无效格式', () => {
      assert(!isValidVersionFormat('invalid'), '文本应为无效')
      assert(!isValidVersionFormat(''), '空字符串应为无效')
    })
  })
})
