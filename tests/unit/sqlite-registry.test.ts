...
describe('SQLite 注册表', () => {
  ...
  describe('注册表条目结构', () => {
    it('应包含所有必需字段', () => {
      ...
      assert(typeof entry.name === 'string', '应包含 name')
      assert(typeof entry.filePath === 'string', '应包含 filePath')
      assert(typeof entry.created === 'string', '应包含 created 时间戳')
    })

    it('应支持可选的 lastVerified 字段', () => {
      const entry = {
        name: 'testdb',
        filePath: '/path/to/test.sqlite',
        created: new Date().toISOString(),
      ...
      assert(typeof entry.lastVerified === 'string', '应支持 lastVerified')
    })
  })

  describe('注册表结构', () => {
    it('应包含 version 和 entries 数组', () => {
      const registry = {
        version: 1 as const,
        entries: [],
      }

      assertEqual(registry.version, 1, 'version 应为 1')
      assert(Array.isArray(registry.entries), '应包含 entries 数组')
    })
  })
  ...
  describe('注册表文件操作', () => {
    it('首次加载时应创建包含空条目的注册表', async () => {
      ...
      assert(existsSync(testRegistryPath), '注册表文件应存在')
    })
    ...
    it('应优雅地处理损坏的注册表', () => {
      ...
      try {
        ...
        // 解析错误时应返回空的 entries
        assertEqual(fallback.entries.length, 0, '发生错误时应返回空条目')
      }
    })
  })
  ...
  describe('条目管理', () => {
    it('应向注册表添加条目', () => {
      ...
      assertEqual(registry.entries.length, 1, '应包含一个条目')
      assertEqual(registry.entries[0].name, 'testdb', '条目名称应匹配')
    })
    ...
    it('应按名称移除条目', () => {
      ...
      assertEqual(registry.entries.length, 1, '移除后应剩一个条目')
      assertEqual(registry.entries[0].name, 'other', '剩余条目应为 "other"')
    })
    ...
    it('应按名称查找条目', () => {
      ...
      assert(found !== undefined, '应找到条目')
      assertEqual(found?.filePath, '/path/a.sqlite', '应具有正确的路径')
    })
    ...
    it('对不存在的条目应返回 null', () => {
      ...
      assert(found === undefined, '不应找到不存在的条目')
    })
  })

  describe('孤立条目检测', () => {
    it('应检测文件不存在的条目', async () => {
      const entries = [
      ...
      assertEqual(orphans.length, 1, '应找到一个孤立条目')
      assertEqual(orphans[0].name, 'missing', '孤立条目应为缺失文件对应的条目')
    })

    it('所有文件都存在时应返回空数组', async () => {
      ...
      assertEqual(orphans.length, 0, '应找不到孤立条目')
    })
    ...
    it('应移除孤立条目', async () => {
      ...
      assertEqual(removedCount, 1, '应移除一个孤立条目')
      assertEqual(registry.entries.length, 1, '应剩余一个条目')
      assertEqual(registry.entries[0].name, 'exists', '剩余条目应存在')
    })
  })
  ...
  describe('路径注册', () => {
    it('应检测路径是否已注册', () => {
      ...
      assert(isRegistered, '应检测到已注册的路径')
    })
    ...
    it('应按文件路径查找条目', () => {
      ...
      assert(found !== undefined, '应按路径找到条目')
      assertEqual(found?.name, 'testdb', '应具有正确的名称')
    })
  })

  describe('更新操作', () => {
    it('应更新 lastVerified 时间戳', () => {
      const entry = {
        name: 'testdb',
        filePath: '/path/to/test.sqlite',
      ...
      assert(entry.lastVerified > entry.created, 'lastVerified 应比 created 新')
    })
    ...
    it('应更新 filePath', () => {
      ...
      assertEqual(entry.filePath, newPath, '路径应已更新')
    })
  })

  describe('重定位操作', () => {
    it('重定位数据库时应更新 filePath', () => {
      const registry = {
        entries: [
          {
            name: 'testdb',
      ...
      assertEqual(registry.entries[0].filePath, newPath, '文件路径应已更新')
    })
    ...
    it('更新路径时应保留条目的其他字段', () => {
      ...
      assertEqual(registry.entries[0].name, 'testdb', '名称应保留')
      assertEqual(
    ...
    it('应处理重定位到不同目录的情况', () => {
      ...
      assert(entry.filePath.includes('project-b'), '应位于新目录')
      assert(!entry.filePath.includes('project-a'), '不应位于旧目录')
    })
    ...
    it('应处理重定位并更改文件名的情况', () => {
      ...
      assert(entry.filePath.endsWith('new-name.sqlite'), '应包含新文件名')
    })
    ...
    it('应处理重定位到主目录的情况', () => {
      ...
      assert(entry.filePath.startsWith('/Users/bob'), '应位于主目录中')
    })
  ...
  describe('连接字符串格式', () => {
    it('应格式化 SQLite 连接字符串', () => {
      const filePath = '/Users/test/mydb.sqlite'
      const connectionString = `sqlite://${filePath}`

      assert(connectionString.startsWith('sqlite://'), '应以 sqlite:// 开头')
      assert(connectionString.includes(filePath), '应包含文件路径')
    })
    ...
    it('应处理包含空格的路径', () => {
      ...
      assert(connectionString.includes('my database'), '应处理空格')
    })
  })

  describe('错误消息', () => {
    it('应为重复名称提供清晰的错误提示', () => {
      const errorMessage = 'SQLite 容器 "testdb" 已存在'

      assert(errorMessage.includes('已存在'), '应指示重复')
      assert(errorMessage.includes('testdb'), '应包含名称')
    })
  })
})
...
describe('SQLite 引擎注册表（config.json 结构）', () => {
  describe('包含 ignoreFolders 的注册表结构', () => {
    it('应包含 version、entries 数组和 ignoreFolders 对象', () => {
      const registry = {
        version: 1 as const,
        entries: [],
      ...
      assert(
        ...
        '应包含 ignoreFolders 对象',
      )
    })

    it('应以键值对 true 的形式存储忽略的文件夹', () => {
      const registry = {
        version: 1 as const,
        entries: [],
        ignoreFolders: {
      ...
      assert('/path/to/folder1' in registry.ignoreFolders, '应包含 folder1')
      assert('/path/to/folder2' in registry.ignoreFolders, '应包含 folder2')
    })
  })
  ...
  describe('忽略文件夹操作', () => {
    it('应将文件夹添加到忽略列表', () => {
      ...
      assert('/path/to/folder' in ignoreFolders, '文件夹应在忽略列表中')
    })
    ...
    it('应从忽略列表中移除文件夹', () => {
      ...
      assert(!('/path/to/folder' in ignoreFolders), '文件夹应从忽略列表中移除')
    })

    it('应为忽略的文件夹提供 O(1) 查询', () => {
      const ignoreFolders: Record<string, true> = {
        '/path/a': true,
        '/path/b': true,
        '/path/c': true,
      ...
      assert(isIgnored, '应以 O(1) 找到文件夹')
    })

    it('对未忽略的文件夹应返回 false', () => {
      const ignoreFolders: Record<string, true> = {
        '/path/a': true,
      }

      const isIgnored = '/path/b' in ignoreFolders
      assert(!isIgnored, '对未忽略的文件夹应返回 false')
    })
    ...
    it('应列出所有忽略的文件夹', () => {
      ...
      assertEqual(folders.length, 3, '应有 3 个文件夹')
      assert(folders.includes('/path/a'), '应包含 path/a')
      assert(folders.includes('/path/b'), '应包含 path/b')
      assert(folders.includes('/path/c'), '应包含 path/c')
    })
  })
})
...
describe('SQLite 扫描器', () => {
  describe('deriveContainerName', () => {
    it('应移除 .sqlite 扩展名', () => {
      ...
      assertEqual(base, 'mydb', '应移除 .sqlite 扩展名')
    })

    it('应移除 .sqlite3 扩展名', () => {
      const fileName = 'mydb.sqlite3'
      const base = fileName.replace(/\.(sqlite3?|db)$/i, '')
      assertEqual(base, 'mydb', '应移除 .sqlite3 扩展名')
    })

    it('应移除 .db 扩展名', () => {
      const fileName = 'mydb.db'
      const base = fileName.replace(/\.(sqlite3?|db)$/i, '')
      assertEqual(base, 'mydb', '应移除 .db 扩展名')
    })

    it('应将无效字符替换为连字符', () => {
      const name = 'my database'
      const sanitized = name.replace(/[^a-zA-Z0-9_-]/g, '-')
      assertEqual(sanitized, 'my-database', '应将空格替换为连字符')
    })

    it('如果以数字开头应添加 db- 前缀', () => {
      const name = '123test'
      const prefixed = /^[a-zA-Z]/.test(name) ? name : 'db-' + name
      assertEqual(prefixed, 'db-123test', '应添加 db- 前缀')
    })

    it('如果以字母开头不应添加前缀', () => {
      const name = 'mytest'
      const prefixed = /^[a-zA-Z]/.test(name) ? name : 'db-' + name
      assertEqual(prefixed, 'mytest', '不应添加前缀')
    })

    it('应移除连续的连字符', () => {
      const name = 'my--database'
      const cleaned = name.replace(/-+/g, '-')
      assertEqual(cleaned, 'my-database', '应移除连续的连字符')
    })
  })
  ...
  describe('未注册文件检测', () => {
    it('应匹配 sqlite 文件扩展名', () => {
      ...
      assertEqual(sqliteFiles.length, 3, '应匹配 3 个 SQLite 文件')
      assert(!sqliteFiles.includes('test.txt'), '不应包含 txt 文件')
    })
    ...
    it('扩展名应不区分大小写', () => {
      ...
      assertEqual(sqliteFiles.length, 3, '应匹配所有大小写变体')
    })
  })
})
  ...
  describe('SQLite 的 ContainerConfig', () => {
    it('基于文件的数据库端口应为 0', () => {
      const config = {
        ...
        created: new Date().toISOString(),
        status: 'running' as const, // "running" 表示文件存在
      }
    ...
    it('应使用文件路径作为 database 字段', () => {
      const filePath = '/path/to/test.sqlite'
      const config = {
        database: filePath,
      }

      assert(config.database.endsWith('.sqlite'), 'database 应为文件路径')
    })

    it('文件存在时应使用 "running" 状态', () => {
      const status = 'running' // 文件存在
      assertEqual(status, 'running', '文件存在时状态应为 running')
    })

    it('文件缺失时应使用 "stopped" 状态', () => {
      const status = 'stopped' // 文件缺失
      assertEqual(status, 'stopped', '文件缺失时状态应为 stopped')
    })
  })
})
